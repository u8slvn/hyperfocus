...
def _resolve_style(style: str) -> Any:
    tags: dict[str, int | None] = {}

    for tag in style.split(" "):
        if tag in _text_styles:
            tags.update({tag: True})
            continue

        try:
            tags.update({"fg": _ansi_colors[tag]})
        except KeyError:
...
def _parse_markup(markup) -> str:
    tags = _resolve_style(markup.group("style"))
