...
def test_pop(test_database):
    ...
    stash_box.pop(task1.id)

    assert len(daily_tracker.get_tasks()) == 1
    assert stash_box.tasks_count == 1
    tasks = stash_box.get_tasks()
    assert tasks[0].uuid == task2.uuid
    assert tasks[0].id == 1


def test_pop_raise_if_task_does_not_exist(test_database):
    date = datetime.date(2022, 1, 1)
    daily_tracker = DailyTracker.from_date(date=date)
    stash_box = StashBox(daily_tracker)

    with pytest.raises(StashBoxError, match=r"Task 1 does not exist in stash box."):
        stash_box.pop(1)
