...
from hyperfocus.commands.task import CopyCommand, TaskCommand
...
def test_task_command_check_task_id_or_ask(
    mocker, test_session, printer, daily_tracker
):
...
def test_task_command_show_tasks(mocker, test_session, printer, daily_tracker):
    ...
    printer.tasks.assert_called_once_with(tasks=[mocker.sentinel.task], newline=False)


def test_task_command_get_task(test_session, daily_tracker):
    daily_tracker.get_task.return_value = Task(id=1, title="foo", details="bar")

    task = TaskCommand(test_session).get_task(1)
...
def test_task_command_get_task_fails(test_session, daily_tracker):
    daily_tracker.get_task.return_value = None

    with pytest.raises(TaskError, match=r"Task \d does not exist."):
        _ = TaskCommand(test_session).get_task(1)
...
def test_copy_command(test_session, printer, pyperclip, task_command):
...
def test_copy_command_fails(test_session, printer, pyperclip, task_command):
